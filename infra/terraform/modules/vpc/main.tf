...
resource "aws_flow_log" "main" {
  ...
  tags = {
    Name        = "${var.environment}-brain-storm-flow-log"
    Environment = var.environment
  }
}
...
resource "aws_cloudwatch_log_group" "flow_log" {
  count             = var.enable_flow_logs ? 1 : 0
  name              = "/vpc/${var.environment}-brain-storm/flow-logs"
  retention_in_days = 30

  tags = {
    Name        = "${var.environment}-brain-storm-flow-logs"
    Environment = var.environment
  }
}
...
resource "aws_iam_role" "flow_log" {
  count = var.enable_flow_logs ? 1 : 0
  name  = "${var.environment}-brain-storm-flow-log-role"
...
resource "aws_iam_role_policy" "flow_log" {
  count = var.enable_flow_logs ? 1 : 0
  name  = "${var.environment}-brain-storm-flow-log-policy"
